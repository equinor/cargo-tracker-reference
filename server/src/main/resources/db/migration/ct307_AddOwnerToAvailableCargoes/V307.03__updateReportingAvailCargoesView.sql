ALTER VIEW ct_reporting.available_cargoes_view AS
SELECT 
	aca.id, 
	aca.month, 
...
FROM ct_reporting.analytics_cargo_availability aca
	...
	LEFT JOIN ct.company c ON aca.equity_owner_id = c.id
UNION
(SELECT
	null, 
	year_month, 
...
GROUP BY year_month, grade, trading_area, equity_owner, equinor_cargo);